...
# Naming?
The name of this repo is a vestige of its original purpose. It was a chaotic pack of functions polluting the global namespace. Some were useful, a lot were only useful in very specific cases, and others were straight up obscure Number Theory sequence generators. Even the functions themselves had extra unnecessary features, like `factorial` having extra args to compute multifactorials, and a boolean arg for switching between "inverse mode" or "standard mode", and support for negative integer inputs where the sequence of return values is the same standard sequence but with alternating sign (-1, 2, -6, 24, ...)
