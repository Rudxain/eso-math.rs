...
{
   'use strict';
   [xprA, xprB, scan, step, pix] = ['('+ xprA +')', '('+ xprB +')', scan.map(x => Number(x) || 0), Number(step) || 1, typeof pix === 'array' ? pix.join('') : String(pix)];
...
//TO-DO: fix the bug that happens when `pix` contains "'"
