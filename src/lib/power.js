...
1 is odd, so it's not a power of 2.
It's a trivial power, because 1 is a power of any Real num
...
export const isPow2 = x => {
	...
	if (isIntN(x)) return !(/**@type {bigint}*/(x) & (/**@type {bigint}*/(x) - 1n))
	return isInt(lb(/**@type {number}*/(x)))
}
